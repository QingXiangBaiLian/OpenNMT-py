...
class PoincareCopyGenerator(nn.Module):
    """An implementation of pointer-generator networks
    :cite:`DBLP:journals/corr/SeeLM17`.
    ...
    * :math:`p_{copy}` the probility of copying a particular word.
    ...
    def __init__(self, input_size, embedding):
        super(PoincareCopyGenerator, self).__init__()
        self.linear_copy = nn.Linear(input_size, 1)
        self.embedding = embedding
        emb_size = self.embedding.shape[1]
        self.poincare_map = PoincareReparametrize(input_size, emb_size)
        self.poincare_emb_map = PoincareReparametrize(emb_size, emb_size)
    ...
    def forward(self, hidden, attn, src_map):
        ...
        # Original probabilities.
        poincare_decoding = self.poincare_map(hidden)
        poincare_embedding =  self.poincare_emb_map(self.embedding)
        logits =  PoincareReparametrize.poincare_dist(poincare_decoding, poincare_embedding)
        # logits[:, self.pad_idx] = -float('inf')
        prob = torch.softmax(logits, 1)
